...
<head>
    ...
    <style>
        ...
        #contacts { width: 160px; border-right: 1px solid #ccc; overflow-y: autoa; background: #fff; transition: background 0.3s, color 0.3s; }
...
def send():
    ...
    # Server-side logging
    print(f"\n📩 Message sent from {data['sender']} to {data['receiver']}")
    print(f"🔓 Plaintext: {data['plaintext']}")
    print(f"🔐 Encrypted AES Key: {data['enc_key'][:60]}...")  # Truncate for readability
    print(f"🧊 Ciphertext: {data['ciphertext'][:60]}...")      # Truncate for readability
    print(f"🧪 IV: {data['iv']}\n")
